...
| Prompt-to-Prompt | `prompt_to_prompt` | В выводе формируется готовый промт, который передаётся в LLM для решения задачи |
...
Режим «Команда» выполняет 4 обращения к API (3 параллельных + саммари), поэтому токены
в ответе суммируются по всем вызовам. Неизвестное или отсутствующее значение
трактуется как `direct`.
